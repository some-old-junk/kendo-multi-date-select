...
export class IndexPage {
  ...
  public selectDatesWithClicks(dates: Date[]): Command<void> {
    return dates.reduce(
      (command, date) => (
        command.then<any>(
          () => this.remote.findByCssSelector(IndexPage.daySel(date)).then<void>(this.mouseClick)
        )
      ),
      this.remote
    );
  ...
  public closeCalendar(): Command<void> {
    const code = () => {
      $('#multi-date-select').data('kendoMultiDateSelect').close();
    };

    return this.remote.execute(code, []).sleep(IndexPage.calendarDelay);
  }

  public toggle() {
    const code = () => {
      $('#multi-date-select').data('kendoMultiDateSelect').toggle();
    };
  ...
  public limits(min: Date, max: Date): Command<void> {
    const code = (stringMin: string, stringMax: string) => {
      const widget = $('#multi-date-select').data('kendoMultiDateSelect');

      widget.min(new Date(stringMin));
      widget.max(new Date(stringMax));
    };

    return this.remote.execute<void>(code, [min, max]);
  }

  public enable(enable: boolean): Command<void> {
    const code = (isEnabled: boolean) => {
      $('#multi-date-select').data('kendoMultiDateSelect').enable(isEnabled);
    };

    return this.remote.execute<void>(code, [enable]);
  }
  ...
  public readonly(readonly: boolean): Command<void> {
    const code = (isReadonly: boolean) => {
      ...
      $('#multi-date-select').data('kendoMultiDateSelect').readonly(isReadonly);
    };

    return this.remote.execute<void>(code, [readonly]);
  }

  public inputDatesInMultiSelect(dates: Date[]): Command<void> {
    const elemCommand = this.remote.findByCssSelector('#multi-date-select input');

    return dates.reduce(
      (command: Command<void>, date) => (
        command.then<any>(() => elemCommand.type([IndexPage.dateToString(date), keys.ENTER]))
      ),
      elemCommand
    );
  }
  ...
  public inputBackspaceInMultiSelect(): Command<void> {
    return this.remote.findByCssSelector('#multi-date-select input').type(keys.BACKSPACE);
  }
  ...
  public clickDateTagClose(dates: Date[]): Command<void> {
    ...
    return dates.reduce(
      (command, date) => (
        command.then<any>(() => this.remote.execute(code, [date]))
      ),
      this.remote
    );
  }
  ...
  public isCalendarOpened(options = { delayed: false }): Command<boolean> {
    const code = () => $('.k-calendar-container').css('display') !== 'none';

    return this.remote.sleep(options.delayed ? IndexPage.calendarDelay : 0).execute(code, []);
  }

  public isEnabled(): Command<boolean> {
    return this.remote.findByCssSelector('#multi-date-select input').isEnabled();
  }

  public multiSelectIsPresent(): Command<boolean> {
    const code = () => {
      const multiSelect = $('#multi-date-select').data('kendoMultiDateSelect').multiSelect();
  ...
  public multiCalendarIsPresent(): Command<boolean> {
    const code = () => {
      const multiCalendar = $('#multi-date-select').data('kendoMultiDateSelect').multiCalendar();
  ...
  public multiDateSelectIsPresent(): Command<boolean> {
    const code = () => {
      const multiDateSelect = $('#multi-date-select').data('kendoMultiDateSelect');
  ...
  public destroy(): Command<void> {
    const code = () => {
      $('#multi-date-select').data('kendoMultiDateSelect').destroy();
    };

    return this.remote.execute<void>(code, []);
  }
